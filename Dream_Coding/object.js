...
const name1 = 'ellie';  // 기본 타입은 변수 하나에 데이터 하나 할당
const age = 4;  
print(name1, age);
function print(name, age) { // 기본 타입은 인자로 줄 때도 하나씩 준다.
    console.log(name1);
    console.log(age);
}
...
const user2 = user; // 이건 참조(메모리 주소)를 복제한거라서 실제론 같은 메모리를 공유하고 있어서 user2를 수정하면 user1도 수정됨
